...
#define MOECONFIG_X(M) \
    ...
    M(playerBackend, "playerBackend", QString, "embedded", "video", "播放后端:embedded(内嵌 libmpv,QML 界面)/external(外部 mpv 进程,内建 OSC 界面)", "播放", "播放后端", "内嵌:视频在应用窗口内渲染,界面为应用主题的控制层(需 libmpv:Linux 装 libmpv2,Windows 在 PATH 或 exe 旁放 libmpv-2.dll);外部:弹独立 mpv 窗口(全平台,mpv 原生 OSC)。无 libmpv 时强制外部。", Combo, optionsPlayerBackend, validatePlayerBackend) \
