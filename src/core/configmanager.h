...
#define MOECONFIG_X(M) \
    ...
    M(backdropMaxWidth, "backdropMaxWidth", QString, "1920", "detail", "详情页背景图请求宽度上限(original=原图)", "详情页", "背景图清晰度", "按屏幕分辨率选:1080p 屏 1920 足够,4K 屏选 3840;原图流量与内存开销大(换图期新旧双帧并存)。重启应用后生效(缓存按档分键,旧档自动过期,无需手动清理)。", Combo, optionsBackdropMaxWidth, validateBackdropMaxWidth) \
    M(detailTextWidth, "textWidth", int, 280, "detail", "标题+介绍区宽度下限(像素;随窗宽生长,720 封顶;默认 280)", "详情页", "文字区最小宽度", "标题+介绍区宽度下限(px),窗口变宽时随窗宽线性增大、720 封顶;默认 280。", Field, nullptr, validatePositiveInt) \
    ...
    M(deviceId, "deviceId", QString, "", "network", "本机设备唯一标识(首启自动生成;改动会让服务器视作新设备)", "", "", "", Hidden, nullptr, nullptr) \
